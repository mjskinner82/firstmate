...
| `fm-principal-authority.sh` | Consume authenticated Mercury assignments, refuse relay captain claims, and own canonical task lifecycle receipts |
| `fm-principal-session-authority.sh` | Locally record direct captain decisions Firstmate already received in its trusted interactive session |
...
| `fm-agent-grade.sh`      | Record recurring maintenance-agent outcomes and report per-agent rates               |
...
| `fm-git-lib.sh`          | Shared Git repository identity helpers                                                |
| `fm-tangle-lib.sh`       | Shared primary-checkout tangle classification                                         |
| `fm-timeout-lib.sh`      | Single owner of hard-bounded command execution and its fallback watchdog |
| `fm-supervision-lib.sh`  | Shared in-flight-work-without-fresh-watcher-beacon predicate                         |
| `fm-ff-lib.sh`           | Shared guarded fast-forward helper for origin pulls and local secondmate syncs       |
| `fm-path-lib.sh`         | Shared strict lexical path-descendant predicate and directory-input resolver          |
