...
`fm-git-lib.sh` resolves the default branch from `origin/HEAD`, then local `main` or `master`, and `fm-tangle-lib.sh` classifies a named non-default primary branch as the tangle.
